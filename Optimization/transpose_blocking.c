...
Fijo O0, NSIZE = 512, variar csize => 1,...,NSIZE, multiplicando de a 2
...
OPT=0 ; gcc -O$OPT transpose_blocking.c; MSIZE=512;
for ((csize = 1; csize <= $MSIZE; csize *= 2)); do echo -n $MSIZE $csize; ./a.out $MSIZE $csize; done > data-$OPT.txt

OPT=1 ; gcc -O$OPT transpose_blocking.c; MSIZE=512;
for ((csize = 1; csize <= $MSIZE; csize *= 2)); do echo -n $MSIZE $csize; ./a.out $MSIZE $csize; done > data-$OPT.txt

OPT=2 ; gcc -O$OPT transpose_blocking.c; MSIZE=512;
for ((csize = 1; csize <= $MSIZE; csize *= 2)); do echo -n $MSIZE $csize; ./a.out $MSIZE $csize; done > data-$OPT.txt
